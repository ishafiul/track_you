...
// Configuration interface for creating HTTP client instances
...
export const createAuthenticatedHttpClient = (config: HttpClientConfig): HttpClient => {
    const {
        baseURL,
        headers = {},
        enableAuth = true,
        ...
        httpClient.addErrorInterceptor(async (error) => {
            ...
            if (isUnauthorized && originalRequest && !originalRequest._retry && !isRefreshing) {
                ...
                try {
                    ...
                    const response = await refreshClient.post<RefreshTokenResponse>(refreshTokenEndpoint, {
