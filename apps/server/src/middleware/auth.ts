...
export const authMiddleware = async (c: HonoContext, next: Next) => {
  ...
  await jwtMiddleware(c, async () => {
    ...
    // Try cache first
    let authInfo: SelectAuth | null | undefined = await cache.match(userIdCacheKey)?.then(res => res?.json());
    let user: UserEntity | null | undefined = await cache.match(userCacheKey)?.then(res => res?.json());
    ...
    if (!authInfo || !user) {
      ...
      if (authInfo) {
        ...
        c.executionCtx.waitUntil(cache.put(userIdCacheKey, new Response(JSON.stringify(authInfo), {
          headers: {
      ...
      if (user) {
        c.executionCtx.waitUntil(cache.put(userCacheKey, new Response(JSON.stringify(user), {
          headers: {
            'content-type': 'application/json',
            'cache-control': 'max-age=7200'
          }
